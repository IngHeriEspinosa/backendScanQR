 ...
 *@NScriptType Restlet
 */
 define(["N/record", "N/error", "N/query", "N/search","N/file","N/encode"], function (
  record,
  error,
  query,
  search,
  ...
  //     myHeaders.append("Authorization", "OAuth realm=\"2045380\",oauth_consumer_key=\"6e13e976e10a75b0519f091d99afb158159dacf6b7b8a9cbd0e367034e8bc9b2\",oauth_token=\"7f1b00b6328c3418bb6de9e2f01a1cc5edb2cf5a761bfa2decc3352b480b0991\",oauth_signature_method=\"HMAC-SHA256\",oauth_timestamp=\"1661976954\",oauth_nonce=\"zi7Twny7ykS\",oauth_version=\"1.0\",oauth_signature=\"Xg2SYRgAefIwOQKc9P0AMqjQVNoiHXwkEca3oSLL81c=\"");
  //     myHeaders.append("Cookie", "NS_ROUTING_VERSION=LAGGING");
  ...
  //    var resultado = fetch("https://2045380.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=384&deploy=1&" + recordtype + "", requestOptions)
  ...
  // var puestoVacantes = getData(getPuestoAPlica);
  // console.log(puestoVacantes);
  ...
  //      //Query
  //      var queryPosiones = query.create({
  //         type: 'customlist_mc_posiciones'
  //     })

  //     //Condicion
  //     var firstCondition = queryPosiones.createCondition({
  //         fieldId: 'isinactive',
  //         operator: query.Operator.IS,
  //         values: false
  //     })

  //     queryPosiones.condition = queryPosiones.and(
  //         firstCondition
  //     )

  //     //Crear columnas
  //     queryPosiones.columns = [
  //         queryPosiones.createColumn({
  //             fieldId: 'id'
  //         }),
  //         queryPosiones.createColumn({
  //             fieldId: 'name'
  ...
  //     //Ordenar
  //     queryPosiones.sort = [
  //         queryPosiones.createSort({
  //             column: queryPosiones.columns[0]

  //         })
  //     ]

  //     var runQuery = queryPosiones.run()
  //     var results = runQuery.results;
  ...
  //         posiciones[i] = {
  //             id: results.value[i].values[0],
  //             name: results.value[i].values[1],
  //         }
  ...
  //          log.debug("getPuestoAPlica", error.message)
  //          return "{error:" + error.message  + "}";

  //     }

  //     log.debug("Datos Posiciones", posiciones)
  //     return  posiciones;
  ...
  var createRecord = {
    solicitudEmpleo: (data) => {
      ...
      try {
        ...
        solicitud.setValue({ fieldId: "custrecord_mc_sexo", value: data.sexo });
        solicitud.setValue({
        ...
        // solicitud.setValue({
        //   fieldId: "custrecord_mc_empresa_simil",
        //   value: data.nombreEmpresaS,
        // });
        // solicitud.setValue({
        //   fieldId: "custrecord_mc_departamento_empresa_simil",
        //   value: data.cboDptoEmpresaS,
        // });
        // solicitud.setValue({
        //   fieldId: "custrecord_mc_area_empresa_simil",
        //   value: data.cboAreaEmpresaS,
        // });
        // solicitud.setValue({
        //   fieldId: "custrecord_mc_nivel_empresa_simil",
        //   value: data.cboNivelEmpresaS,
        // });
        // solicitud.setValue({
        //   fieldId: "custrecord_mc_posicion_empresa_simil",
        //   value: data.posicionEmpresaS,
        // });
        solicitud.setText({
        ...
        // Ejemplo: Guardar el archivo de imagen en el File Cabinet
       
        // var base64EncodedFile = encode.convert({
        ...
        // var fileData = data.curriculum.getContents();s
        // solicitud.setValue({
        //   fieldId: 'custrecord_mc_cv_adjunto',
        //   value: base64EncodedFile,
        //   fileType: file.Type.PDF,
        //   filename: 'curriculum.pdf'+ data.nombre,
        // });
        


        //errorEnviandoData = 'custrecord_mc_solicitud_otra_posicion_mc' + ' ' + data.nacionalidad;
        // Llenar hijos
        ...
        for (var i = 0; i < data.rowsExp.length; i++) {
          ...
          // errorEnviandoData = 'nivelEmpresa';
          // var nivelEmpresa = data.rowsExp[i].nivelEmpresa.split("-"); 
          // var codigoNivel = nivelEmpresa[0].trim();

          // var areaEmpresa = data.rowsExp[i].areaEmpresa.split("-");
          // var codigoArea = areaEmpresa[0].trim(); 
          ...
          solicitud.setCurrentSublistValue({
            ...
            value: data.rowsExp[i].fechaSalida ? new Date(data.rowsExp[i].fechaSalida) : '',
          });
          solicitud.setCurrentSublistValue({
            sublistId: "recmachcustrecord_mc_persona_solicitante_exp",
          ...
          solicitud.setCurrentSublistValue({
            ...
            value: data.rowsExp[i].salarioFinal ? data.rowsExp[i].salarioFinal : 0,
          });
          solicitud.setCurrentSublistValue({
            sublistId: "recmachcustrecord_mc_persona_solicitante_exp",
        ...
        // //errorEnviandoData = solicitud;
        // //LLEnar Certificaciones
        for (var i = 0; i < data.rowsCertifi.length; i++) {
          solicitud.selectNewLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_certi",
          });
          ...
          solicitud.setCurrentSublistValue({
            ...
            value: data.rowsCertifi[i].fechaCaducidad ? new Date(data.rowsCertifi[i].fechaCaducidad): '',
          });
        ...
        for (var i = 0; i < data.rowsRefes.length; i++) {
          ...
          solicitud.selectNewLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_refes",
          });
          solicitud.selectLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_refes",
            line: i,
        ...
        for (var i = 0; i < data.rowsEduca.length; i++) {
          ...
          solicitud.selectNewLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_educa",
          });
          solicitud.selectLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_educa",
            line: i,
          ...
          solicitud.setCurrentSublistValue({
            ...
            value: data.rowsEduca[i].fechaTermino ? new Date(data.rowsEduca[i].fechaTermino) : '',
          });
          solicitud.setCurrentSublistValue({
            sublistId: "recmachcustrecord_mc_persona_solicitante_educa",
        ...
        for (var i = 0; i < data.rowsSkills.length; i++) {
          ...
          solicitud.selectNewLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_skills",
          });
          solicitud.selectLine({
            sublistId: "recmachcustrecord_mc_persona_solicitante_skills",
            line: i,
    ...
    createPedidoVenta: (data) => {
      try {
        ...
        solicitud.setValue({ fieldId: "otherrefnum", value: data.otherrefnum });

        solicitud.setValue({ fieldId: "memo", value: data.memo });
        solicitud.setValue({ fieldId: "salesrep", value: data.salesrep }); //Op
        solicitud.setValue({
        ...
        for (var i = 0; i < data.detalle.length; i++) {
          ...
          solicitud.setCurrentSublistValue({
            ...
            value: data.detalle[i].recmachcustrecord_mc_persona_solicitante,
          });
          solicitud.setCurrentSublistValue({
            sublistId: "recmachcustrecord_mc_persona_solicitante",
            fieldId: "quantity",
    ...
    getPedidos: (id) => {
      ...
      recmachcustrecord_mc_persona_solicitantesQuery.sort = [
        recmachcustrecord_mc_persona_solicitantesQuery.createSort({
          column: recmachcustrecord_mc_persona_solicitantesQuery.columns[0],
          ascending: false,
        }),
      ];

      //Ejecutar el query pra extrar los resultados;
      let resulSetTerm = recmachcustrecord_mc_persona_solicitantesQuery.run();

      //Asignar resultados;
      let resultsTerm = resulSetTerm.results;

      //recorrer resultados;
      // for (var i = 0; i < resultsTerm.length; i++) {

      //     term[i] = {
      //         id: resultsTerm[i].values[0],
      //         name: resultsTerm[i].values[1],
      //         daysuntilnetdue: resultsTerm[i].values[2]
      //     }
